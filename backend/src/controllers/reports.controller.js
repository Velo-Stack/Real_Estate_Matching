...
const shapeArabic = (text) => {
    ...
    if (arabicReshaper) {
      try {
        if (typeof arabicReshaper === "function")
          shaped = arabicReshaper(shaped);
        else if (typeof arabicReshaper.reshape === "function")
          shaped = arabicReshaper.reshape(shaped);
      } catch (e) {
        /* ignore */
      }
    }
  ...
  if (
    bidiJs &&
    typeof bidiJs.getEmbeddingLevels === "function" &&
    typeof bidiJs.reorderVisual === "function"
  ) {
    try {
  ...
  try {
    ...
    if (type === "offers") {
      ...
      offers.forEach((offer) => {
        const useArabic =
          hasArabic(offer.city) ||
          hasArabic(offer.district) ||
          hasArabic(offer.createdBy?.name);
        ...
        row.eachCell((cell) => {
          if (typeof cell.value === "string")
            cell.value = shapeAndReorder(cell.value);
      ...
      matches.forEach((match) => {
        const useArabic =
          hasArabic(match.offer.city) || hasArabic(match.request.city);
        const row = worksheet.addRow({
          id: match.id,
...
const exportPDF = async (req, res) => {
  try {
    ...
    const titles = {
      offers: "تقرير العروض",
      requests: "تقرير الطلبات",
      matches: "تقرير نتائج المطابقة",
    };
    const titleText = arabicFont
      ? titles[type]
    ...
    const dateLabel = hasArabic(titleText)
      ? " الإنشاء تاريخ : " + wrapLRI(dateStrRaw)
      : "Generated on: " + dateStrRaw;
    ...
    if (type === "offers") {
      ...
      offers.forEach((offer, index) => {
        ...
        const line1 = shapeAndReorder(
          `${indexStr}. ${offer.type} - ${offer.city}، ${offer.district}`,
        );

        const priceStr = formatRange(offer.priceFrom, offer.priceTo);
        const priceSeg = wrapLRI(`${priceStr} SAR`);
        const areaSeg = wrapLRI(
          `${formatNumber(offer.areaFrom)} - ${formatNumber(offer.areaTo)} m²`,
        );
        ...
        doc
          ...
          .text(
            shapeAndReorder(
              `   ${labelUsage}: ${offer.usage} | ${labelLand}: ${offer.landStatus}`,
            ),
            { align: "right" },
          );
        doc
          .fontSize(10)
          .text(`   ${labelPrice}: ${priceSeg}`, { align: "right" });
        doc
          .fontSize(10)
          .text(`   ${labelArea}: ${areaSeg}`, { align: "right" });
        doc
          .fontSize(10)
          .text(
            `   ${labelBroker}: ${shapeAndReorder(offer.createdBy?.name || "")}`,
            { align: "right" },
          );
        doc.moveDown();
      });
      ...
      requests.forEach((reqData, index) => {
        const indexStr = wrapLRI(String(index + 1));
        const line1 = shapeAndReorder(
          `${indexStr}. ${reqData.type} - ${reqData.city}`,
        );
      ...
      matches.forEach((match, index) => {
        const indexStr = wrapLRI(String(index + 1));
        const line1 = shapeAndReorder(
          `${indexStr}. Offer: ${match.offer.type} (${match.offer.city}) <-> Request: ${match.request.type} (${match.request.city})`,
        );
        ...
        doc
          ...
          .text(
            shapeAndReorder(
              `   Score: ${wrapLRI(match.score)} | Status: ${wrapLRI(match.status)}`,
            ),
            { align: "right" },
          );
        doc.moveDown();
